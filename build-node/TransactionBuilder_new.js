...
var TransactionBuilder = function () {
    ...
    */
    ...
    TransactionBuilder.prototype.get_type_operation = function get_type_operation(name, operation) {
        ...
        // console.info("operation_instance",operation_instance);
    ...
    TransactionBuilder.prototype.set_required_fees = function set_required_fees(asset_id, removeDuplicates) {
        ...
        for (var i = 0, op; i < this.operations.length; i++) {
            ...
            // console.info('this.operations[i]',JSON.parse(JSON.stringify(this.operations[i])));
            var opObject = _serializer.ops.operation.toObject(op);
            // console.info('opObject',opObject,JSON.parse(JSON.stringify(this.operations)));
        ...
        promises.push(Promise.all(feeAssets.map(function (id) {
            // console.info('[operations, id]',[operations, id]);

            // console.info('_bcxjsWs.Apis.instance().db_api().exec("get_required_fees", [operations, id])',_bcxjsWs.Apis.instance().db_api().exec("get_required_fees", [operations, id]));
        ...
        */
        ...
        if (feeAssets.length > 1 || feeAssets[0] !== "1.3.0") {
            ...
            */
            ...
            // console.info('_bcxjsWs.Apis.instance().db_api().exec("get_required_fees", [operations, "1.3.0"])',JSON.parse(JSON.stringify(_bcxjsWs.Apis.instance().db_api().exec("get_required_fees", [operations, "1.3.0"]))));
        ...
        return Promise.all(promises).then(function (results) {
            ...
            */
            var allFees = results[0],
                ...
                // console.info('allFees',JSON.parse(JSON.stringify(results)));
            ...
            */
            ...
            allFees.forEach(function (feeSet) {
                var filteredFeeSet = feeSet.map(function (f) {
                    // console.info("filteredFeeSet f",f);
                    if (Array.isArray(f)) {
            ...
            function checkPoolBalance(map) {
                ...
                var _loop2 = function _loop2(asset) {
                    ...
                    // console.info('feePool',feePool,feePoolMap);
                    ...
                    if (map[asset].total > feePool) {
                        ...
                        map[asset].ops.forEach(function (opIndex) {
                            // console.info('coreFees',coreFees,opIndex,map[asset]);
                            if (coreFees[opIndex].length === 2 && "propIdx" in map[asset]) {
                                map[asset].propIdx.forEach(function (prop_idx) {
                                    final_fees[prop_idx] = proposalFeesByAsset[asset][prop_idx];
                                });
                            } else {
                                // console.info('feesByAsset',feesByAsset);
                ...
                // console.info('final_fees',final_fees);
                return final_fees;
            }

            // console.info('feeMap',JSON.parse(JSON.stringify(feeMap)));
    ...
    TransactionBuilder.prototype.get_potential_signatures = function get_potential_signatures() {
        ...
        //   console.info('get_potential_signatures results',results);
    ...
    TransactionBuilder.prototype.broadcast = function broadcast(was_broadcast_callback,isCallback=false) {
        var _this4 = this;
        if (this.tr_buffer) {
            return this._broadcast(was_broadcast_callback,isCallback);
        } else {
            return this.finalize().then(function () {
                return _this4._broadcast(was_broadcast_callback,isCallback);
            });
        }
    };
...
function _broadcast(was_broadcast_callback,isCallback) {
    var _this5 = this;
    ...
    return new Promise(function (resolve, reject) {
        ...
        // console.info("isCallback: ", isCallback);
        if(isCallback){
            ...
            _bcxjsWs.Apis.instance().network_api().exec("broadcast_transaction", [tr_object]).then(function (res) {
                let _debug_time=new Date().getTime()-_this5.debug_time;
                // document.querySelector(".div_transferAsset .fee").innerText=_debug_time;
                // alert(_debug_time);
                // console.info("debug_time",new Date().getTime()-_this5.debug_time)
